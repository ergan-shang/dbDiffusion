...
def load_data(
    ...
    assert adata.X.shape[0] == all_class_emb.shape[0] # cell number should be same
    # preprocess the data. modify this part if use your own dataset. the gene expression must first norm1e4 then log1p
    ...
    ########## You may want to change this part since we are not going to use labelled [0, 1, 2, 3...]. Instead we use the embedding
    classes = adata.obs['celltype'].values
    label_encoder = LabelEncoder()
    labels = classes
    label_encoder.fit(labels)  # change to {'pert_A': 0, 'pert_B': 1.....}
    classes = label_encoder.transform(
        labels)  # change to [0, 1, 2, 1, ...] from ['pert_A', 'pert_B', 'pert_C', 'pert_B', ...]
    ...
    if not train_vae:
        ...
        cell_data = autoencoder(torch.tensor(cell_data, dtype=torch.float32).cuda(), return_latent=True)
...
class CellDataset(Dataset):
    def __init__(
        ...
        self.all_class_emb = all_class_emb

    def __len__(self):
        return self.data.shape[0]

    def __getitem__(self, idx):
        arr = torch.tensor(self.data[idx], dtype=torch.float32)
        class_emb = torch.tensor(self.all_class_emb[idx], dtype=torch.float32)
        out_dict = {}
        if self.class_name is not None:
            out_dict["y"] = np.array(self.class_name[idx], dtype=np.int64)
        return arr, out_dict, class_emb

# DataLoader will auotomatically use the function "__getitem__" with idx being [0, ..., 11] the first batch, [12, ...23] the second time...
# so that cond is a dictionary instead of a numpy array containing class_label. This dict has only one key, 'y'
